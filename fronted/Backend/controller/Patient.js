...
export const insertPatient=(req,res)=>{
    const query="INSERT INTO `patient`(`name`, `mobile_no`, `gender`, `address`, `dob`, `entry_by`, `status`, `entry_date`,`password`) Values(?)";
    const values= [
        req.body.name,
        req.body.mobile_no,
        req.body.gender,
...
export const updatePatient=(req,res)=>{
  ...
  //  console.log(doctor_id)
    const query="UPDATE `patient` SET `name`=?,`mobile_no`=?,`gender`=?,`address`=?,`dob`=?,`entry_by`=?,`status`=?,`update_date`=? where patient_id=?"
    const values= [        
        req.body.name,
        req.body.mobile_no,
        req.body.gender,
